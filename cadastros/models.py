...
class Produto(models.Model):
    ...
    description = models.CharField(max_length=500, blank=True, verbose_name='descricao')
...
class ProductShip(models.Model):
    user = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    idshipping = models.ForeignKey(Shipping, on_delete=models.SET_NULL, null=True, blank=True, verbose_name='transportadoras')
    product = models.ForeignKey(Produto, on_delete=models.SET_NULL, null=True, blank=True, verbose_name='produto')
    price = models.DecimalField(max_digits=20, decimal_places=2, verbose_name='preco')
    deliverydays = models.IntegerField(verbose_name='dias de entrega')  # corrigido

    def __str__(self):
        return self.product.title if self.product else "Produto não definido"
